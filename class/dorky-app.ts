...
import * as encryption from '../dorky_modules/encryption';
...
import { hexToUint8Array, uint8ArrayToHex } from '../dorky_modules/uint8array-extras';
...
export class DorkyApp {
  static FLAG_ENCRYPTED = 'data_encrypted';
  static LNDHUB = 'lndhub';
  static DO_NOT_TRACK = 'donottrack';
  static HANDOFF_STORAGE_KEY = 'HandOff';

  private static _instance: DorkyApp | null = null;

  static keys2migrate = [DorkyApp.HANDOFF_STORAGE_KEY, DorkyApp.DO_NOT_TRACK];
  ...
  static getInstance(): DorkyApp {
    if (!DorkyApp._instance) {
      DorkyApp._instance = new DorkyApp();
    }

    return DorkyApp._instance;
  }
  ...
  async migrateKeys() {
    ...
    for (const key of DorkyApp.keys2migrate) {
      try {
  ...
  storageIsEncrypted = async (): Promise<boolean> => {
    let data;
    try {
      data = await this.getItemWithFallbackToRealm(DorkyApp.FLAG_ENCRYPTED);
    } catch (error: any) {
      console.warn('error reading `' + DorkyApp.FLAG_ENCRYPTED + '` key:', error.message);
      return false;
    }
  ...
  encryptStorage = async (password: string): Promise<void> => {
    ...
    await this.setItem(DorkyApp.FLAG_ENCRYPTED, '1');
  };
  ...
  async saveToDisk(): Promise<void> {
    ...
    try {
      ...
      await this.setItem(DorkyApp.FLAG_ENCRYPTED, this.cachedPassword ? '1' : '');

      // now, backing up same data in realm:
      const realmkeyValue = await this.openRealmKeyValue();
      this.saveToRealmKeyValue(realmkeyValue, 'data', JSON.stringify(data));
      this.saveToRealmKeyValue(realmkeyValue, DorkyApp.FLAG_ENCRYPTED, this.cachedPassword ? '1' : '');
      realmkeyValue.close();
    } catch (error: any) {
  ...
  isHandoffEnabled = async (): Promise<boolean> => {
    try {
      return !!(await AsyncStorage.getItem(DorkyApp.HANDOFF_STORAGE_KEY));
    } catch (_) {}
    return false;
  };

  setIsHandoffEnabled = async (value: boolean): Promise<void> => {
    await AsyncStorage.setItem(DorkyApp.HANDOFF_STORAGE_KEY, value ? '1' : '');
  };

  isDoNotTrackEnabled = async (): Promise<boolean> => {
    try {
      const keyExists = await AsyncStorage.getItem(DorkyApp.DO_NOT_TRACK);
      if (keyExists !== null) {
        const doNotTrackValue = !!keyExists;
        if (doNotTrackValue) {
          await DefaultPreference.set(DorkyApp.DO_NOT_TRACK, '1');
          AsyncStorage.removeItem(DorkyApp.DO_NOT_TRACK);
        } else {
          return Boolean(await DefaultPreference.get(DorkyApp.DO_NOT_TRACK));
        }
      }
    } catch (_) {}
    const doNotTrackValue = await DefaultPreference.get(DorkyApp.DO_NOT_TRACK);
  ...
  setDoNotTrack = async (value: boolean) => {
    if (value) {
      ...
      await DefaultPreference.clear(DorkyApp.DO_NOT_TRACK);
    }
  };
