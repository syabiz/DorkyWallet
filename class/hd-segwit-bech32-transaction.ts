...
import { isUint8Array, uint8ArrayToHex } from '../dorky_modules/uint8array-extras';
...
export class HDSegwitBech32Transaction {
  ...
  async _fetchTxhexAndDecode() {
    ...
    const hexes = await DorkyElectrum.multiGetTransactionByTxid([this._txid], false, 10);
  ...
  async _fetchRemoteTx() {
    const result = await DorkyElectrum.multiGetTransactionByTxid([this._txid || this._txDecoded!.getId()], true);
  ...
  async getInfo() {
    ...
    const prevTransactions = await DorkyElectrum.multiGetTransactionByTxid(prevInputs, true);
