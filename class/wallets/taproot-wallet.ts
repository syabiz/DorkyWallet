...
export class TaprootWallet extends SegwitBech32Wallet {
  ...
  static scriptPubKeyToAddress(scriptPubKey: string): string | false {
    try {
      ...
      return bitcoin.address.fromOutputScript(publicKey, DORKCOIN_TAPROOT);
    } catch (_) {
      return false;
    }
  }
  ...
  getAddress(): string | false {
    ...
    let address;
    try {
      const keyPair = ECPair.fromWIF(this.secret, DORKCOIN_TAPROOT);
  ...
  createTransaction(
    ...
    const psbt = new bitcoin.Psbt({ network: DORKCOIN_TAPROOT });
