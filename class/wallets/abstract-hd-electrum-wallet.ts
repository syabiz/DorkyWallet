...
import { ElectrumHistory } from '../../dorky_modules/DorkyElectrum';
import ecc from '../../dorky_modules/noble_ecc';
import { hexToUint8Array, concatUint8Arrays, uint8ArrayToHex } from '../../dorky_modules/uint8array-extras';
...
import { DORKCOIN, DORKCOIN_TAPROOT } from '../../dorky_modules/networks';
import { DORKCOIN, DORKCOIN_TAPROOT } from '../../dorky_modules/networks';
...
export class AbstractHDElectrumWallet extends AbstractHDWallet {
  ...
  async fetchTransactions() {
    ...
    const histories = await DorkyElectrum.multiGetHistoryByAddress(addresses2fetch);
    ...
    const txdatas = await DorkyElectrum.multiGetTransactionByTxid(Object.keys(txs), true);
  ...
  async _binarySearchIterationForInternalAddress(index: number) {
    ...
    for (let c = 0; c < Math.round(index / this.gap_limit); c++) {
      const histories = await DorkyElectrum.multiGetHistoryByAddress(gerenateChunkAddresses(c));
      // @ts-ignore
  ...
  async _binarySearchIterationForBIP47Address(paymentCode: string, index: number) {
    ...
    let lastHistoriesWithUsedAddresses = null;
    for (let c = 0; c < Math.round(index / this.gap_limit); c++) {
      const histories = await DorkyElectrum.multiGetHistoryByAddress(generateChunkAddresses(c));
      // @ts-ignore
      if (this.constructor._getTransactionsFromHistories(histories).length > 0) {
        // in this particular chunk we have used addresses
        lastChunkWithUsedAddressesNum = c;
  ...
  async _fetchBalance() {
    ...
    const txs = await DorkyElectrum.multiGetHistoryByAddress(lagAddressesToFetch); // <------ electrum call
    ...
    const balances = await DorkyElectrum.multiGetBalanceByAddress(addresses2fetch);
  ...
  async fetchUtxo(): Promise<void> {
    ...
    const fetchedUtxo = await DorkyElectrum.multiGetUtxoByAddress(addressess);
    this._utxo = [];
    ...
    for (const u of this._utxo) {
      ...
      if (!u.confirmations && u.height) u.confirmations = DorkyElectrum.estimateCurrentBlockheight() - u.height;
    }
      ...
      for (const output of tx.outputs) {
        ...
        if (ownedAddressesHashmap[String(address)]) {
          ...
          utxos.push({
            ...
            wif: false,
            height: DorkyElectrum.estimateCurrentBlockheight() - (tx.confirmations ?? 0),
          });
        }
      }
    }
  ...
  createTransaction(
    ...
    let psbt = new bitcoin.Psbt({ network: DORKCOIN });
    let c = 0;
  ...
  _addPsbtInput(psbt: Psbt, input: CoinSelectReturnInput, sequence: number, masterFingerprintBuffer: Uint8Array) {
    ...
    const p2wpkh = bitcoin.payments.p2wpkh({ pubkey, network: DORKCOIN });
    if (!p2wpkh.output) {
  ...
  /**
   * Creates Segwit Bech32 Dorkcoin address
   */
  ...
  /**
   * Creates Segwit P2SH Dorkcoin address
   */
  _nodeToP2shSegwitAddress(hdNode: BIP32Interface): string {
    const { address } = bitcoin.payments.p2sh({
      redeem: bitcoin.payments.p2wpkh({ pubkey: hdNode.publicKey, network: DORKCOIN }),
      network: DORKCOIN,
    });
  ...
  async wasEverUsed(): Promise<boolean> {
    const txs1 = await DorkyElectrum.getTransactionsByAddress(this._getExternalAddressByIndex(0));
    if (txs1.length > 0) {
      return true;
    }
    if (!this.allowBIP47()) {
    ...
    if (!["m/84'/4151811'/0'", "m/44'/4151811'/0'", "m/49'/4151811'/0'"].includes(this.getDerivationPath() as string)) {
      return false;
    }

    const bip47_instance = this.getBIP47FromSeed();
    const address = bip47_instance.getNotificationAddress();
    const txs2 = await DorkyElectrum.getTransactionsByAddress(address);
    ...
    for (let cc = 0; cc < psbt.inputCount; cc++) {
      ...
      if (!psbt.inputHasHDKey(cc, hdRoot)) {
        for (const derivation of psbt.data.inputs[cc].bip32Derivation || []) {
          ...
          const keyPair = ECPair.fromWIF(wif, DORKCOIN);
          try {
  ...
  async fetchBIP47SenderPaymentCodes(): Promise<void> {
    const bip47_instance = this.getBIP47FromSeed();
    const address = bip47_instance.getNotificationAddress();
    const histories = await DorkyElectrum.multiGetHistoryByAddress([address]);
    const txHashes = histories[address].map(({ tx_hash }) => tx_hash);

    const txHexs = await DorkyElectrum.multiGetTransactionByTxid(txHashes, false);
