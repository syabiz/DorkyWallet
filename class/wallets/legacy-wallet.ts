...
import { hexToUint8Array, concatUint8Arrays } from '../../dorky_modules/uint8array-extras';
...
export class LegacyWallet extends AbstractWallet {
  ...
  getAddress(): string | false {
    ...
    let address;
    try {
      const keyPair = ECPair.fromWIF(this.secret, DORKCOIN);
  ...
  async fetchUtxo(): Promise<void> {
    try {
      const address = this.getAddress();
      if (!address) throw new Error('LegacyWallet: Invalid address');
      const utxos = await DorkyElectrum.multiGetUtxoByAddress([address]);
      this._utxo = [];
      ...
      const txhexes = await DorkyElectrum.multiGetTransactionByTxid(
  ...
  async broadcastTx(txhex: string): Promise<boolean> {
    const broadcast = await DorkyElectrum.broadcastV2(txhex);
  ...
  createTransaction(
    ...
    const psbt = new bitcoin.Psbt({ network: DORKCOIN });
    let c = 0;
    ...
    if (!skipSigning) {
      ...
      keyPair = ECPair.fromWIF(this.secret, DORKCOIN); // secret is WIF
    }
   ...
   * @see https://github.com/DorkyWallet/DorkyWallet/issues/3394
  ...
  async wasEverUsed(): Promise<boolean> {
    ...
    const txs = await DorkyElectrum.getTransactionsByAddress(address);
    return txs.length > 0;
  }
}
