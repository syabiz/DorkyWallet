...
import { concatUint8Arrays, hexToUint8Array, uint8ArrayToHex } from '../../dorky_modules/uint8array-extras';
...
export class HDAezeedWallet extends AbstractHDElectrumWallet {
  ...
  static readonly derivationPath = "m/84'/4151811'/0'";
  // @ts-ignore: override
  ...
  _getNode0() {
    const root = bip32.fromSeed(this._getEntropyCached());
    const node = root.derivePath("m/84'/4151811'/0'");
  ...
  _getWIFByIndex(internal: boolean, index: number): string | false {
    ...
    const root = bip32.fromSeed(this._getEntropyCached());
    const path = `m/84'/4151811'/0'/${internal ? 1 : 0}/${index}`;
