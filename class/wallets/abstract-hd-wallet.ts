...
import * as bip39custom from '../../dorky_modules/bip39';
...
export class AbstractHDWallet extends LegacyWallet {
  ...
  async getAddressAsync(): Promise<string> {
    ...
    for (c = 0; c < this.gap_limit + 1; c++) {
      ...
      let txs = [];
      try {
        txs = await DorkyElectrum.getTransactionsByAddress(address);
      } catch (Err: any) {
        console.warn('DorkyElectrum.getTransactionsByAddress()', Err.message);
      }
      if (txs.length === 0) {
        // found free address
        freeAddress = address;
