...
import { hexToUint8Array, uint8ArrayToHex } from '../../dorky_modules/uint8array-extras/index';
import assert from 'assert';
import ecc from '../../dorky_modules/noble_ecc.ts';
...
export class LightningArkWallet extends LightningCustodianWallet {
  ...
  _getIdentity() {
    ...
    if (!this._privateKeyCache) {
      ...
      const path = `m/86'/4151811'/${accountNumber}'/${internal}/${index}`;
  ...
  async _initLightningSwaps() {
    ...
    this._limitMin = feesResponseJson?.ARK?.DORK?.limits?.minimal ?? 333;
    this._limitMax = feesResponseJson?.ARK?.DORK?.limits?.maximal ?? 1000000;
    this._feePercentage = feesResponseJson?.ARK?.DORK?.fees?.percentage ?? 0;
    if (!feesResponseJson?.ARK?.DORK?.fees?.percentage) {
  ...
  async payInvoice(invoice: string, freeAmount: number = 0) {
    ...
    if (this.isAddressValid(invoice)) {
      ...
      await this._wallet.sendDorkcoin({
        address: invoice,
        amount: freeAmount,
      });
      return;
