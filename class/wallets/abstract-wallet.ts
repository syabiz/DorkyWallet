...
export class AbstractWallet {
  ...
  preferredBalanceUnit: DorkcoinUnit;
  chain: Chain;
  hideBalance: boolean;
  ...
  constructor() {
    ...
    this.preferredBalanceUnit = DorkcoinUnit.DORK;
  ...
  getPreferredBalanceUnit(): DorkcoinUnit {
    for (const value of Object.values(DorkcoinUnit)) {
      if (value === this.preferredBalanceUnit) {
        return this.preferredBalanceUnit;
      }
    }
    return DorkcoinUnit.DORK;
  }

  setPreferredBalanceUnit(unit: DorkcoinUnit): void {
    if (Object.values(DorkcoinUnit).includes(unit)) {
  ...
  setSecret(newSecret: string): this {
    ...
    if (m && m.length === 3) {
      ...
      if (derivationPath.startsWith("m/84'/4151811'/") && this.secret.toLowerCase().startsWith('xpub')) {
        // need to convert xpub to zpub
        this.secret = this._xpubToZpub(this.secret.split('/')[0]);
      }

      if (derivationPath.startsWith("m/49'/4151811'/") && this.secret.toLowerCase().startsWith('xpub')) {
    ...
    if (!this._derivationPath) {
      if (this.secret.startsWith('xpub')) {
        this._derivationPath = "m/44'/4151811'/0'"; // Assume default BIP44 path for legacy wallets
      } else if (this.secret.startsWith('ypub')) {
        this._derivationPath = "m/49'/4151811'/0'"; // Assume default BIP49 path for segwit wrapped wallets
      } else if (this.secret.startsWith('zpub')) {
        this._derivationPath = "m/84'/4151811'/0'"; // Assume default BIP84 for native segwit wallets
      }
    }
