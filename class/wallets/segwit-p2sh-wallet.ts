...
import { hexToUint8Array } from '../../dorky_modules/uint8array-extras';
...
function pubkeyToP2shSegwitAddress(pubkey: Uint8Array): string | false {
  const { address } = bitcoin.payments.p2sh({
    redeem: bitcoin.payments.p2wpkh({ pubkey, network: DORKCOIN }),
    network: DORKCOIN,
  });
...
export class SegwitP2SHWallet extends LegacyWallet {
  ...
  createTransaction(
    ...
    inputs.forEach(input => {
      ...
      const p2sh = bitcoin.payments.p2sh({ redeem: p2wpkh, network: DORKCOIN });
      if (!p2sh.output) {
