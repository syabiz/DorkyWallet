...
export class LightningCustodianWallet extends LegacyWallet {
  ...
  preferredBalanceUnit = DorkcoinUnit.SATS;
  ...
  async createAccount(isTest: boolean = false) {
    const response = await fetch(this.baseURI + '/create', {
      method: 'POST',
      body: JSON.stringify({ partnerid: 'dorkywallet', accounttype: (isTest && 'test') || 'common' }),
  ...
  async fetchBalance(noRetry?: boolean): Promise<void> {
    ...
    if (!json.DORK || typeof json.DORK.AvailableBalance === 'undefined') {
      throw new Error('API unexpected response: ' + JSON.stringify(json));
    }

    this.balance = json.DORK.AvailableBalance;
