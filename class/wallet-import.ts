...
import bip39WalletFormatsDorkyWallet from './bip39_wallet_formats_dorkywallet.json';
...
bip39WalletFormatsElectrum.push({
  ...
  derivation_path: "m/86'/4151811'/0'",
  script_type: 'p2tr',
...
const startImport = (
  ...
  async function* importGenerator() {
    ...
    const hd2 = new HDLegacyP2PKHWallet();  // Default to BIP44 Legacy as per DorkyWallet spec
    hd2.setSecret(text);
    if (password) {
    ...
    if (hd2.validateMnemonic()) {
      ...
      const searchPaths = searchAccounts ? bip39WalletFormatsElectrum : bip39WalletFormatsDorkyWallet;
      ...
      // if we havent found any wallet for this seed suggest new BIP44 Legacy wallet (default for DorkyWallet)
      if (!walletFound) {
        yield { wallet: hd2 };
      }
    }
    ...
    if (segwitWallet.getAddress()) {
      ...
      // if no wallets was ever used, import all of them (Legacy/BIP44 first as default)
      if (!walletFound) {
        yield { wallet: legacyWallet };
        yield { wallet: segwitBech32Wallet };
        yield { wallet: segwitWallet };
