...
const RateExtractors = {
  Coinbase: async (ticker: string): Promise<number> => {
    try {
      const json = (await fetchRate(`https://api.coinbase.com/v2/prices/DORK-${ticker.toUpperCase()}/buy`)) as CoinbaseResponse;
  ...
  CoinDesk: async (ticker: string): Promise<number> => {
    try {
      const json = (await fetchRate(
        `https://min-api.cryptocompare.com/data/price?fsym=DORK&tsyms=${ticker.toUpperCase()}`,
      )) as CoinDeskResponse;
  ...
  BNR: async (): Promise<number> => {
    try {
      ...
      if (matches && matches[1]) {
        ...
        // Convert DORK to RON using the USD to RON exchange rate
  ...
  YadioConvert: async (ticker: string): Promise<number> => {
    try {
      const json = (await fetchRate(`https://api.yadio.io/convert/1/DORK/${ticker}`)) as YadioConvertResponse;
