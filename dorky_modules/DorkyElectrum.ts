...
import loc from '../loc';
import { GROUP_IO_DORKYWALLET } from './currency';
...
const defaultPeer = { host: 'electrumx.dorkcoin.org', ssl: 50002 };
...
export async function connectMain(): Promise<void> {
  ...
  try {
    ...
    const ver = await mainClient.initElectrum({ client: 'dorkywallet', version: '1.4' });
    if (ver && ver[0]) {
...
export const getBalanceByAddress = async function (address: string): Promise<{ confirmed: number; unconfirmed: number }> {
  try {
    ...
    const script = bitcoin.address.toOutputScript(address, DORKCOIN);
...
export const multiGetBalanceByAddress = async (addresses: string[], batchsize: number = 200): Promise<MultiGetBalanceResponse> => {
  ...
  for (const chunk of chunks) {
    ...
    for (const addr of chunk) {
      const script = bitcoin.address.toOutputScript(addr, DORKCOIN);
      const hash = bitcoinjs_crypto_sha256(script);
