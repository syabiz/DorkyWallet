...
import { DorkyApp as DorkyAppClass } from '../class/dorky-app';
import prompt from '../helpers/prompt';
import { showKeychainWipeAlert } from '../hooks/useBiometrics';
import loc from '../loc';

const DorkyApp = DorkyAppClass.getInstance();
...
export const startAndDecrypt = async (retry?: boolean, passwordPrompt?: PasswordPromptCallback): Promise<boolean> => {
  ...
  if (DorkyApp.getWallets().length > 0) {
    return true;
  }
  await DorkyApp.migrateKeys();
  let password: undefined | string;
  if (await DorkyApp.storageIsEncrypted()) {
    if (passwordPrompt) {
  ...
  try {
    success = await DorkyApp.loadFromDisk(password);
  } catch (error) {
