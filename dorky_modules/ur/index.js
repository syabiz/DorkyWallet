...
function decodeUR(arg) {
  ...
  if (!decoder.isComplete()) {
    throw new Error("decodeUR func can't work with multimart BC-UR data. Prease use DorkyURDecoder instead.");
  }
...
class DorkyURDecoder extends URDecoder {
  ...
  toString() {
    ...
    if (decoded.type === 'crypto-account') {
      ...
      for (const outputDescriptor of cryptoAccount.outputDescriptors) {
        ...
        if (derivationPath.startsWith("m/49'/4151811'/")) {
          // converting to ypub
        ...
        if (derivationPath.startsWith("m/44'/4151811'/")) {
          // converting to xpub
          let data = b58.decode(result.ExtPubKey);
          data = data.slice(4);
