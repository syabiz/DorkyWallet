...
const TransactionDetails = () => {
  ...
  const handleOnOpenTransactionOnBlockExplorerTapped = () => {
    const url = `${selectedBlockExplorer.url}/transaction/${tx?.hash}`;
  ...
  const handleCopyPress = (stringToCopy: string) => {
    Clipboard.setString(stringToCopy !== actionKeys.CopyToClipboard ? stringToCopy : `${selectedBlockExplorer.url}/transaction/${tx?.hash}`);
  };
  ...
  return (
    <SafeAreaScrollView>
      <HandOffComponent
        ...
        url={`${selectedBlockExplorer.url}/transaction/${tx.hash}`}
      />
      <BlueCard>
        <View>
          <TextInput
