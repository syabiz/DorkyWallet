...
import { satoshiToLocalCurrency } from '../dorky_modules/currency';
 ...
 * @param withFormatting {boolean} Works only with `DorkcoinUnit.SATS`, makes spaces wetween groups of 000
 * @returns {string}
 */
export function formatBalance(balance: number, toUnit: string, withFormatting = false): string {
  if (toUnit === undefined) {
    return balance + ' ' + loc.units[DorkcoinUnit.DORK];
  }
  if (toUnit === DorkcoinUnit.DORK) {
    const value = new BigNumber(balance).dividedBy(100000000).toFixed(8);
    return removeTrailingZeros(value) + ' ' + loc.units[DorkcoinUnit.DORK];
  } else if (toUnit === DorkcoinUnit.SATS) {
    return (withFormatting ? new Intl.NumberFormat().format(balance).toString() : String(balance)) + ' ' + loc.units[DorkcoinUnit.SATS];
  } else {
 ...
 *
 * @param balance {number} Satoshis
 * @param toUnit {string} Value from models/bitcoinUnits.js, for example `DorkcoinUnit.SATS`
 * @param withFormatting {boolean} Works only with `DorkcoinUnit.SATS`, makes spaces wetween groups of 000
 * @returns {string}
 */
...
/**
 * @see https://github.com/DorkyWallet/DorkyWallet/issues/3466
 */
