package org.dorkcoin.dorkywallet
...
object MarketAPI {
    ...
    suspend fun fetchPrice(context: Context, currency: String): String? {
        Log.i(TAG, "Fetching Dorkcoin price for currency: $currency")
    ...
    private fun buildURLString(source: String, endPointKey: String): String {
        return if (baseUrl != null) {
            ...
            when (source) {
                ...
                "YadioConvert" -> "https://api.yadio.io/convert/1/DORK/$endPointKey"
                "Exir" -> "https://api.exir.io/v1/ticker?symbol=dork-irt"
                "coinpaprika" -> "https://api.coinpaprika.com/v1/tickers/dork-dorkcoin?quotes=$endPointKey"
                "Bitstamp" -> "https://www.bitstamp.net/api/v2/ticker/dork${endPointKey.lowercase()}"
                "Coinbase" -> "https://api.coinbase.com/v2/prices/DORK-${endPointKey.uppercase()}/buy"
                "CoinGecko" -> "https://api.coingecko.com/api/v3/simple/price?ids=dorkcoin&vs_currencies=${endPointKey.lowercase()}"
                "BNR" -> "https://www.bnr.ro/nbrfxrates.xml"
                "Kraken" -> "https://api.kraken.com/0/public/Ticker?pair=XDORKZ${endPointKey.uppercase()}"
                "CoinDesk" -> "https://min-api.cryptocompare.com/data/price?fsym=DORK&tsyms=${endPointKey.uppercase()}"
                else -> "https://min-api.cryptocompare.com/data/price?fsym=DORK&tsyms=${endPointKey.uppercase()}"
            }
        }
    }
    ...
    private fun parseJSONBasedOnSource(jsonString: String, source: String, endPointKey: String): String? {
        return try {
            ...
            when (source) {
                ...
                "CoinGecko" -> json.getJSONObject("dorkcoin").getString(endPointKey.lowercase())
                "Exir" -> json.getString("last")
                "Bitstamp" -> json.getString("last")
                "coinpaprika" -> json.getJSONObject("quotes").getJSONObject(endPointKey).getString("price")
                "Coinbase" -> json.getJSONObject("data").getString("amount")
                "Kraken" -> json.getJSONObject("result").getJSONObject("XDORKZ${endPointKey.uppercase()}").getJSONArray("c").getString(0)
                "CoinDesk" -> {
        ...
        try {
            ...
            if (priceStr != null) {
                ...
                if (rate > 0) {
                    ...
                    // Calculate sats - convert to integer for display (Dorkcoin uses 10^8 like Bitcoin)
                    val satsValue = ((10 / rate) * 100000000).toInt()
