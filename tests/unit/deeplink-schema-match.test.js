...
jest.mock('../../dorky_modules/DorkyElectrum', () => {
  return {
    connectMain: jest.fn(),
  };
});
...
describe.each(['', '//'])('unit - DeepLinkSchemaMatch', function (suffix) {
  it('hasSchema', () => {
    ...
    assert.ok(DeeplinkSchemaMatch.hasSchema(`dorkywallet:bitcoin:${suffix}12eQ9m4sgAwTSQoNXkRABKhCXCsjm2jdVG`));
    assert.ok(DeeplinkSchemaMatch.hasSchema(`dorkywallet:bitcoin:${suffix}bc1qh6tf004ty7z7un2v5ntu4mkf630545gvhs45u7?amount=666&label=Yo`));
    assert.ok(DeeplinkSchemaMatch.hasSchema(`dorkywallet:bitcoin:${suffix}BC1QH6TF004TY7Z7UN2V5NTU4MKF630545GVHS45U7?amount=666&label=Yo`));
    assert.ok(DeeplinkSchemaMatch.hasSchema(`dorkywallet:BITCOIN:${suffix}BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE`));
    assert.ok(DeeplinkSchemaMatch.hasSchema(`dorkywallet:BITCOIN:${suffix}BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE?amount=666&label=Yo`));
    assert.ok(
      DeeplinkSchemaMatch.hasSchema(
        `dorkywallet:lightning:${suffix}`,
      ),
    );
  });

  it('isDorkcoin Address', () => {
    assert.ok(DeeplinkSchemaMatch.isDorkcoinAddress('12eQ9m4sgAwTSQoNXkRABKhCXCsjm2jdVG'));
    assert.ok(DeeplinkSchemaMatch.isDorkcoinAddress('3GcKN7q7gZuZ8eHygAhHrvPa5zZbG5Q1rK'));
    assert.ok(DeeplinkSchemaMatch.isDorkcoinAddress('bc1qykcp2x3djgdtdwelxn9z4j2y956npte0a4sref'));
    assert.ok(DeeplinkSchemaMatch.isDorkcoinAddress('BC1QYKCP2X3DJGDTDWELXN9Z4J2Y956NPTE0A4SREF'));
    assert.ok(DeeplinkSchemaMatch.isDorkcoinAddress(`bitcoin:${suffix}BC1QH6TF004TY7Z7UN2V5NTU4MKF630545GVHS45U7`));
    assert.ok(DeeplinkSchemaMatch.isDorkcoinAddress(`BITCOIN:${suffix}BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE`));
    assert.ok(DeeplinkSchemaMatch.isDorkcoinAddress(`BITCOIN:${suffix}BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE?amount=666&label=Yo`));
  });
  ...
  it('isBoth Dorkcoin & Invoice', () => {
    assert.ok(
      DeeplinkSchemaMatch.isBothDorkcoinAndLightning(
    ...
    const rez = DeeplinkSchemaMatch.isBothDorkcoinAndLightning(
      `bitcoin:${suffix}BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE?amount=0.000001&lightning=lnbc1u1pwry044pp53xlmkghmzjzm3cljl6729cwwqz5hhnhevwfajpkln850n7clft4sdqlgfy4qv33ypmj7sj0f32rzvfqw3jhxaqcqzysxq97zvuq5zy8ge6q70prnvgwtade0g2k5h2r76ws7j2926xdjj2pjaq6q3r4awsxtm6k5prqcul73p3atveljkn6wxdkrcy69t6k5edhtc6q7lgpe4m5k4`,
    );
    ...
    const rez2 = DeeplinkSchemaMatch.isBothDorkcoinAndLightning(
    ...
    const rez3 = DeeplinkSchemaMatch.isBothDorkcoinAndLightning(
    ...
    // no amount
    const rez4 = DeeplinkSchemaMatch.isBothDorkcoinAndLightning(
      `bitcoin:bc1q8flg3jcnv6x6mpjrqty8h8h9mg0shgp5jc9smk?lightning=`,
    );
  ...
  it('navigationForRoute', async () => {
    const events = [
      ...
      {
        argument: { url: `dorkywallet:BITCOIN:${suffix}BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE?amount=666&label=Yo` },
        expected: [
          'SendDetailsRoot',
          { screen: 'SendDetails', params: { uri: 'BITCOIN:BC1Q3RL0MKYK0ZRTXFMQN9WPCD3GNAZ00YV9YP0HXE?amount=666&label=Yo' } },
        ],
      ...
      {
        argument: {
          url: `dorkywallet:lightning:${suffix}`,
        },
        expected: [
          'ScanLNDInvoiceRoot',
          {
      ...
      {
        argument: {
          url: 'dorkywallet:setelectrumserver?server=electrum1.dorkywallet.io%3A443%3As',
        },
        expected: [
          'ElectrumSettings',
          {
            server: 'electrum1.dorkywallet.io:443:s',
          },
        ],
      },
      {
        argument: {
          url: 'dorkywallet:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com',
        },
        expected: [
          'LightningSettings',
          {
  ...
  it('decodes bip21', () => {
    ...
    decoded = DeeplinkSchemaMatch.bip21decode(
      `bitcoin:${suffix}bc1qnapskphjnwzw2w3dk4anpxntunc77v6qrua0f7?amount=0.0001&pj=https://btc.donate.kukks.org/DORK/pj`,
    );
    assert.strictEqual(decoded.options.pj, 'https://btc.donate.kukks.org/DORK/pj');
  ...
  it('can decodeDorkcoinUri', () => {
    assert.deepStrictEqual(
      DeeplinkSchemaMatch.decodeDorkcoinUri(
        `bitcoin:${suffix}bc1qnapskphjnwzw2w3dk4anpxntunc77v6qrua0f7?amount=0.0001&pj=https://btc.donate.kukks.org/DORK/pj`,
      ),
      {
        ...
        amount: 0.0001,
        memo: '',
        payjoinUrl: 'https://btc.donate.kukks.org/DORK/pj',
      },
    );

    assert.deepStrictEqual(
      DeeplinkSchemaMatch.decodeDorkcoinUri(`BITCOIN:${suffix}1BgGZ9tcN4rm9KBzDn7KprQz87SZ26SAMH?amount=20.3&label=Foobar`),
      {
        address: '1BgGZ9tcN4rm9KBzDn7KprQz87SZ26SAMH',
        amount: 20.3,
        memo: 'Foobar',
  ...
  it('can work with some deeplink actions', () => {
    ...
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('dorkywallet:setelectrumserver?server=electrum1.dorkywallet.io%3A443%3As'),
      'electrum1.dorkywallet.io:443:s',
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('setelectrumserver?server=electrum1.dorkywallet.io%3A443%3As'),
      'electrum1.dorkywallet.io:443:s',
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('ololo:setelectrumserver?server=electrum1.dorkywallet.io%3A443%3As'),
      false,
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getServerFromSetElectrumServerAction('setTrololo?server=electrum1.dorkywallet.io%3A443%3As'),
      false,
    );

    assert.strictEqual(
      DeeplinkSchemaMatch.getUrlFromSetLndhubUrlAction('dorkywallet:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com'),
      'https://lndhub.herokuapp.com',
    );
    assert.strictEqual(
      DeeplinkSchemaMatch.getUrlFromSetLndhubUrlAction('dorkywallet:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com%3A443'),
