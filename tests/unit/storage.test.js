...
it('Appstorage - loadFromDisk works', async () => {
  ...
  await AsyncStorage.setItem(DorkyApp.FLAG_ENCRYPTED, '1');
  const Storage3 = new DorkyApp();
...
it('AppStorage - getTransactions() work', async () => {
  ...
  for (const tx of txs) {
    ...
    assert.strictEqual(tx.walletPreferredBalanceUnit, 'DORK');
  }
