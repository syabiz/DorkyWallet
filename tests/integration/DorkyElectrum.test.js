...
beforeAll(async () => {
  ...
  try {
    await DorkyElectrum.connectMain();
  } catch (err) {
...
describe('DorkyElectrum', () => {
  it('ElectrumClient can estimate fees from histogram', async () => {
    assert.strictEqual(
      DorkyElectrum.calcEstimateFeeFromFeeHistorgam(1, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
    ...
    assert.strictEqual(
      DorkyElectrum.calcEstimateFeeFromFeeHistorgam(18, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
    ...
    assert.strictEqual(
      DorkyElectrum.calcEstimateFeeFromFeeHistorgam(144, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
  ...
  it('ElectrumClient can test connection', async () => {
    assert.ok(!(await DorkyElectrum.testConnection('electrum1.dorkywallet.io', 444, false)));
    assert.ok(!(await DorkyElectrum.testConnection('electrum1.dorkywallet.io', false, 444)));
    assert.ok(!(await DorkyElectrum.testConnection('ya.ru', 444, false)));
    assert.ok(!(await DorkyElectrum.testConnection('google.com', false, 80)));
    assert.ok(!(await DorkyElectrum.testConnection('google.com', 80, false)));
    assert.ok(!(await DorkyElectrum.testConnection('google.com', false, 443)));
    assert.ok(!(await DorkyElectrum.testConnection('google.com', 443, false)));
    assert.ok(!(await DorkyElectrum.testConnection('joyreactor.cc', false, 443)));
    assert.ok(!(await DorkyElectrum.testConnection('joyreactor.cc', 443, false)));
    assert.ok(!(await DorkyElectrum.testConnection('joyreactor.cc', 80, false)));
    assert.ok(!(await DorkyElectrum.testConnection('joyreactor.cc', false, 80)));

    assert.ok(await DorkyElectrum.testConnection('electrum1.dorkywallet.io', '50001'));
    assert.ok(await DorkyElectrum.testConnection('electrum1.dorkywallet.io', false, 443));
  });

  it('ElectrumClient can estimate fees', async () => {
    assert.ok((await DorkyElectrum.estimateFee(1)) >= 1);
    const fees = await DorkyElectrum.estimateFees();
  ...
  it('ElectrumClient can request server features', async () => {
    const features = await DorkyElectrum.serverFeatures();
  ...
  it('DorkyElectrum can do getBalanceByAddress()', async function () {
    const address = '3GCvDBAktgQQtsbN6x5DYiQCMmgZ9Yk8BK';
    const balance = await DorkyElectrum.getBalanceByAddress(address);
  ...
  it('DorkyElectrum can do getTransactionsByAddress()', async function () {
    const txs = await DorkyElectrum.getTransactionsByAddress('bc1qe7q08prc2spln2l7qdvvlcgqxm9za9z7mjnpzc');
  ...
  it.skip('DorkyElectrum can do getMempoolTransactionsByAddress()', async function () {
    const txs = await DorkyElectrum.getMempoolTransactionsByAddress('bc1qp33en9mnw277c9vz5fz9vcu666cvervdnk02327wwph97hdjurqqxtl03c');
    assert.ok(txs.length > 0);
    assert.ok(txs[0].tx_hash);
    assert.ok(txs[0].fee);

    const rez = await DorkyElectrum.multiGetTransactionByTxid([txs[0].tx_hash], 10, true);
    assert.ok(rez[txs[0].tx_hash]);
  });

  it('DorkyElectrum can do getTransactionsFullByAddress()', async function () {
    const txs = await DorkyElectrum.getTransactionsFullByAddress('bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh');
    const tx = txs[0];
  ...
  it('DorkyElectrum can do txhexToElectrumTransaction()', () => {
    const tx =
    ...
    const decoded = DorkyElectrum.txhexToElectrumTransaction(tx);
  ...
  it.each([false, true])('DorkyElectrum can do multiGetBalanceByAddress(), disableBatching=%p', async function (diableBatching) {
    if (diableBatching) DorkyElectrum.setBatchingDisabled();
    const balances = await DorkyElectrum.multiGetBalanceByAddress([
    ...
    if (diableBatching) DorkyElectrum.setBatchingEnabled();
  });

  it('DorkyElectrum can do multiGetUtxoByAddress()', async () => {
    const utxos = await DorkyElectrum.multiGetUtxoByAddress(
      [
        'bc1qe7q08prc2spln2l7qdvvlcgqxm9za9z7mjnpzc',
        'bc1qdsf8p4knu2u0h9cspflh0ftjp8qayve3r5nme8',
        'bc1qzfmm8d9saalnjjnskj2mekycg73hecpajt8urp',
  ...
  it.each([false, true])('ElectrumClient can do multiGetHistoryByAddress(), disableBatching=%p', async disableBatching => {
    if (disableBatching) DorkyElectrum.setBatchingDisabled();
    const histories = await DorkyElectrum.multiGetHistoryByAddress(
      [
    ...
    if (disableBatching) DorkyElectrum.setBatchingEnabled();
  });

  it.each([false, true])('ElectrumClient can do multiGetTransactionByTxid(), disableBatching=%p', async disableBatching => {
    if (disableBatching) DorkyElectrum.setBatchingDisabled();
    const txdatas = await DorkyElectrum.multiGetTransactionByTxid(
      [
        'ad00a92409d8982a1d7f877056dbed0c4337d2ebab70b30463e2802279fb936d',
  ...
  it('multiGetTransactionByTxid() can work with big batches', async () => {
    ...
    const vintxdatas = await DorkyElectrum.multiGetTransactionByTxid(vinTxids, true);
